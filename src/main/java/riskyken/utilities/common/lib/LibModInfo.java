package riskyken.utilities.common.lib;

public class LibModInfo {
	public static final String ID = "RiskyKensUtilities";
	public static final String NAME = "Risky Ken's Utilities";
	public static final String VERSION = "0.0.20";
	public static final String CHANNEL = "RKUtils";
	
	public static final String PROXY_CLIENT_CLASS = "riskyken.utilities.proxies.ClientProxy";
	public static final String PROXY_COMMNON_CLASS = "riskyken.utilities.proxies.CommonProxy";
}
